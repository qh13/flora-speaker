...
`claw_core` writes session history through a typed record batch callback.

In `edge_agent`, `persist_session` is `claw_memory_persist_session_callback` from `claw_memory`.
`claw_core` invokes it for plain turns, intermediate tool rounds, final assistant records, and failure notes so session records are persisted through `claw_memory`.
