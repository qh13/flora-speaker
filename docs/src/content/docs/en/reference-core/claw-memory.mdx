...
- `claw_memory_persist_session_callback`: passed as `claw_core_config_t.persist_session`; typed raw session records are appended to the session file for plain turns, tool rounds, final assistant records, and failure notes.
